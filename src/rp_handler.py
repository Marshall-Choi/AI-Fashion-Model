...
def validate_input(job_input):
    ...
    selected_workflow = job_input.get('workflow_select')
    if selected_workflow is not None:
        workflow_number = workflow_mapping[selected_workflow]
    ...
    if use_automask is not None:
        use_automask_id = [key for key, value in id_to_class_type.items() if value == 'Boolean_Auto_Mask']
        if use_automask_id:
            workflow.get(f'{use_automask_id[0]}').get('inputs')['value'] = use_automask
