...
def num_tokens_consumed_from_request(
    ...
    if api_endpoint == "completions":
        ...
        for message in messages:
            if "content" in message:  # 메시지에 'content' 키가 있는지 확인
                content = message["content"]
                if isinstance(content, list):  # content가 리스트인 경우
                    # 각 메시지의 'content'에서 텍스트를 추출하여 토큰 수를 계산
                    prompt_tokens += sum(len(encoding.encode(item["text"])) for item in content if "text" in item)
                elif isinstance(content, str):  # content가 문자열인 경우
...
def parallel_processor(requests_filepath, save_filepath, api_key):
    # parse command line arguments
    parser = argparse.ArgumentParser()
    parser.add_argument("--requests_filepath", default=requests_filepath)
    parser.add_argument("--save_filepath", default=save_filepath)
